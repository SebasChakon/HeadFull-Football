# HeadFull Football

A fast-paced, arcade-style 3D Football game where players control characters with comically oversized heads. Score goals, trigger power-ups, and dominate the field in chaotic, over-the-top matches.

>  **Status:** Currently under active development.

---

##  Gameplay

HeadFull Football takes the beautiful game and cranks it up to eleven. Players compete in Football matches where the physics of those enormous heads play a central role — whether it's heading the ball with unstoppable force or getting knocked off balance at the worst moment.

- **3D arcade Football** with exaggerated physics and character proportions
- **Power-ups** scattered across the field that shake up the match dynamics
- Fast, fun, and chaotic — built for replayability

---

##  Tech Stack

| Layer         | Technology          |
|---------------|---------------------|
| Engine        | Unity               |
| Language      | C#                  |
| Rendering     | Unity URP           |

---

##  Installation & Setup

### Prerequisites

- [Unity Hub](https://unity.com/download)
- Unity Editor (check the project's `ProjectSettings/ProjectVersion.txt` for the required version)

### Steps

1. **Clone the repository:**
   ```bash
   git clone https://github.com/SebasChakon/Headfull-Football.git
   ```

2. **Open in Unity Hub:**
   - Launch Unity Hub and click **Add project from disk**.
   - Select the cloned repository folder.

3. **Open the project:**
   - Make sure you have the correct Unity version installed (Unity Hub will prompt you if not).
   - Click the project to open it in the Unity Editor.

4. **Run the game:**
   - In the Unity Editor, open the "MainMenu" scene from the `Assets/Scenes` folder.
   - Press the **Play** button to run the game in the editor.

### Building a Standalone Executable

1. Go to **File → Build Settings**.
2. Select your target platform (Windows, macOS, Linux).
3. Click **Build** and choose an output folder.

---

##  License

MIT — free to use, modify, and distribute.
